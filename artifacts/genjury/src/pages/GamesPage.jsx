...
const ACCENT = {
  ...
  plasma: {
    ...
    notify: 'border-plasma/30 bg-plasma/10 text-plasma hover:bg-plasma/18 hover:border-plasma/50',
    notifyActive: 'border-plasma/50 bg-plasma/20 text-plasma',
  },
  signal: {
  ...
  gold: {
    ...
    notify: 'border-gold/30 bg-gold/10 text-gold hover:bg-gold/18 hover:border-gold/50',
    notifyActive: 'border-gold/50 bg-gold/20 text-gold',
  },
}
...
export default function GamesPage() {
  ...
  const handleNotify = (gameId, gameName) => {
    setNotified((prev) => ({ ...prev, [gameId]: true }))
    addToast(`We'll let you know when ${gameName} launches.`, 'success')
  }
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 py-10 sm:py-14">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {GAMES.map((g) => {
          ...
          const isNotified = notified[g.id]
          return (
            <div
              key={g.id}
              className={`group relative glass rounded-2xl border overflow-hidden transition-all duration-300 ${a.border} ${a.glow} ${
                live ? `${a.hoverGlow} hover:-translate-y-0.5` : 'opacity-80'
              }`}
            >
              {/* Top accent gradient */}
              ...
              <div className="relative p-6">
                ...
                <div className="flex items-start gap-4 mb-5">
                  <div className={`w-[52px] h-[52px] rounded-2xl flex items-center justify-center ${a.bg} border ${a.border} ${a.text} transition-transform duration-200 ${live ? 'group-hover:scale-105' : ''} flex-shrink-0`}>
                ...
                {live ? (
                  ...
                  <button
                    onClick={() => !isNotified && handleNotify(g.id, g.name)}
                    disabled={isNotified}
                    className={`w-full py-3 text-sm rounded-xl border inline-flex items-center justify-center gap-2 transition-all duration-200 ${
                      isNotified
                        ? `${a.notifyActive} cursor-default`
                        : `${a.notify} cursor-pointer active:scale-[0.98]`
                    }`}
                    aria-label={isNotified ? `Notified for ${g.name}` : `Notify me when ${g.name} launches`}
                  >
                    {isNotified ? (
                      <>
