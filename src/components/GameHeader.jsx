...
export default function GameHeader() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 glass border-b border-white/[0.07]">
      ...
      {/* ── Row 1: Logo · Phase · Timer ── */}
      <div className="flex items-center h-12 sm:h-16 px-3 sm:px-4 gap-2 sm:gap-4 overflow-hidden">
        ...
        {/* Prize pool — desktop compact badge */}
        {showPot && (
        ...
        {/* New case — desktop only */}
        <button
          onClick={handleLeaveAndCreate}
          title={confirmLeave ? 'Click again — you will leave this game' : 'Open or join a new case'}
          className={`hidden sm:inline-flex flex-shrink-0 items-center gap-1 px-2 py-1 rounded-lg border text-xs font-mono transition-all ${
            confirmLeave
      ...
      {/*
        ── Row 2 (mobile only): Prize · Round · Leave · Avatar ──
        Only visible below sm breakpoint. Keeps the main row clean on phones.
      */}
      <div className="sm:hidden flex items-center h-8 px-3 gap-2 border-t border-white/[0.05] overflow-hidden">
        ...
        <button
          ...
          className={`flex-shrink-0 inline-flex items-center gap-0.5 px-2 py-0.5 rounded-md border text-[10px] font-mono transition-all ${
            confirmLeave
              ? 'border-signal/60 bg-signal/15 text-signal animate-pulse'
              : 'border-neon/25 bg-neon/8 text-neon hover:bg-neon/18'
          }`}
        >
          {confirmLeave
