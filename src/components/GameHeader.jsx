  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 h-16 glass border-b border-ghost-border flex items-center px-4 gap-4">
      ...
      <div className="flex-shrink-0">
        <span className="badge bg-ghost border border-ghost-border text-white/50 font-mono text-xs">
          {roomCode}
        </span>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center">
        <div className="badge bg-plasma/15 text-plasma border border-plasma/30 text-xs tracking-widest">
          {PHASE_LABELS[phase] || ''}
        </div>
        {category && phase !== PHASES.SCOREBOARD && (
          <span className="text-white/30 text-xs font-mono mt-0.5">{category}</span>
        )}
      </div>
