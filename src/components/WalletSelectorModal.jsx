...
import { detectWallets, connectWithProvider, hasInjectedProvider } from '../lib/genlayer'

// ── Inline SVG wallet icons ───────────────────────────────────────────────────
// Kept as raw SVG so there are zero extra assets to fetch.
...
export default function WalletSelectorModal({ onConnect, onClose }) {
  ...
  const handleSelect = async (index) => {
    ...
    setConnecting(index)
    try {
      const addr = await connectWithProvider(index)
      onConnect(addr)
    } catch (e) {
      const msg = e?.message || 'Could not connect wallet'
      if (/rejected|denied|cancel/i.test(msg)) {
        setError('Request cancelled — please approve in your wallet.')
      } else {
        setError(msg)
      }
      setConnecting(null)
          ...
          <div className="p-5 space-y-4">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-white/40 text-xs mt-0.5">Choose a wallet to continue</p>
              </div>
              <button
                onClick={onClose}
            ...
            {/* Error message */}
            {error && (
              <div className="rounded-xl bg-signal/10 border border-signal/30 px-3.5 py-2.5 text-signal text-xs leading-snug">
                {error}
              </div>
            )}
            ...
            {hasInj && wallets.length > 0 ? (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-white/35 text-[11px] font-mono mt-0.5">
                          {busy ? 'Approve in wallet…' : 'EIP-1193'}
                        </div>
                      </div>
              ...
              /* Fallback: ethereum present but detectWallets returned empty — connect directly */
              <button
            ...
            <p className="text-white/22 text-[11px] text-center font-mono leading-relaxed pb-1">
              {hasInj
                ? `EIP-1193 compatible · approve in your wallet`
